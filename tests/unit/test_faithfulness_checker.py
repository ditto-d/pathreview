...
class TestFaithfulnessChecker:
    ...
    def test_short_claim_fully_supported_scores_high(self, checker):
        """A claim with only one meaningful token should score near 1.0 when fully backed
        by context, regression test for issue #152."""
        feedback = "Knows Python "
        context_chunks = [{"text": "Extensive Python expertise"}]
